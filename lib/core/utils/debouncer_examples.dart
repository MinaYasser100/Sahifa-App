...
class _WindowResizeExampleState extends State<WindowResizeExample> {
  ...
  void _updateLayout() {
    print('Updating layout for size: $_windowSize');
  }
...
class ApiRateLimitExample {
  ...
  Future<void> _sendAnalytics(String activity) async {
    _apiCallCount++;
    print('Analytics sent ($_apiCallCount): $activity');
  }
...
class SearchWithRequestIdExample {
  ...
  Future<void> search(String query) async {
    ...
    print(
      '🔍 Search initiated - Query: "$query", Request ID: $currentRequestId',
    );
    ...
    _debouncer.run(() async {
      ...
      if (currentRequestId != _requestId) {
        print('⏭️ Skipping outdated request ID: $currentRequestId');
        return;
      }
      ...
      if (currentRequestId != _requestId) {
        print(
          '⏭️ Discarding outdated results for request ID: $currentRequestId',
        );
        return;
      }

      print('✅ Search success for request ID $currentRequestId: $results');
    });
  }
